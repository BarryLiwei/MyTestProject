...
                    return
                    ...
                    if (_response.Kick) {
                        ...
                        window.eventBus.$emit('c_RLtransferStatus', '3');   //判断线路
                        return;
                    } else {
   ...
   m7AutoBusyTimeChanged: function(autoBusyTime) {
    ...
    dialout: function(phoneNum,type,dccGroupId) {
        ...
        // 开发
        if (href.indexOf('cc-gqsm.dev-chexiu.cn') > -1) {
            env = ',env:dev'
        } else {  // 正式
            env = ',env:prod'
        }

        let transferTelWay = window.localStorage.getItem('c_use_tel')
        let callType = transferTelWay == 3 ? 'sip' : transferTelWay == 2 ? 'gateway' : 'Local'; // 1手机，2SIP，3软电话
        let agentInfo = JSON.parse(window.localStorage.getItem('agentInfo'))
        let roleKey = "roleKey:" + agentInfo.role.role_key;
        let id = ",id:" + agentInfo.admin.id;
        let dccGroupIdParam = dccGroupId ? ',dccGroupId:'+dccGroupId: '';
        let ActionID3 = roleKey + id + ",times:" + new Date().getTime() + dccGroupIdParam + env;
        let smallTel = window.localStorage.getItem('smallTel');
        
        var sendData = {
            FromExten: exten,
            Exten: phoneNum,
            ExtenType: callType,
            DialoutStrVar: ActionID3,
            OutShow: smallTel
        }
        //https的域名https://xiaohaobar.7moor.com
        var account = "T00000011282";//需要调整成自己账号的账户编号
        var secret = "bbe66b30-67f5-11ea-be41-67e701cf4ec3";//需要调整成自己账号的账户秘钥
        ...
        m7$.ajax({
            url: "https://openapis.7moor.com/v20180426/call/dialout/" + account + "?sig="+ sig,
            type: 'post',
        ...
        //     var authorization = Base64.encode(account + ':' + timestamp);
        //     m7$.ajax({
        //         url: "https://apis.7moor.com/v20160818/call/dialout/" + account + "?sig="+ sig,
        //         type: 'post',
        //         data: JSON.stringify(sendData),
        //         dataType:'json',
        //         contentType: "application/json;charset=utf-8",
        ...
        //         Async: "true",
        //         HasCdrVar:true,
        //         ActionID:ActionID3,    //自定义的参数必须是动态变化的
        //         CallType: call_type
        //     },function(response, ioArgs) {
        //         var json = response;
        //         if (!response.Succeed) {
