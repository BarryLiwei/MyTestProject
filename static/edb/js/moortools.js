...
moorCall.moortools.m7BeginLogon = function (loginName, password, extenType, agentStatus) {
    var config = {
        Monitor: false,
        proxy_url: "https://pbx-bj-tx04.7moor.com",//在用户中心查询到的地址，如：https://pbx-bj-ali2.7moor.com
        extenType: extenType,
        password: password,
        User: loginName,
...
moorCall.moortools.input = function(connectType) {
    ...
    if(connectType == "transfer") {
        clickBtn = "<input type=button value='转接' class='btn' style='margin-left:5px;' onclick=\"softphoneBar.m7exTransfer(m7$('#softphone-input').val())\" />";
        keyEvent = "onKeyDown=\"if(event.keyCode == 13){softphoneBar.m7exTransfer(m7$('#softphone-input').val())}\"";
    } else if(connectType == "consult") {
        clickBtn = "<input type=button value='咨询'  class='btn' onclick=\"softphoneBar.m7exConsult(m7$('#softphone-input').val())\" />";
    ...
    var html = "<div style='text-align:right'>" +
        ...
        "<input id='softphone-input' value='' class='ipt' placeholder='请输入手机号或工号' type='text' size='20' class='fl' "+keyEvent+" />" + clickBtn + "</div>" +
        "</div>"
    msgObj.html(html) ;
...
moorCall.moortools.error = function(message) {
    ...
    var html = "<div style='height:27px;padding-top:15px;padding-right:10px;text-align: left'><span style='font-weight: bold;font-size: 14px; margin: 20px'>提示</span><div style='float:right'><a href='javascript:moorCall.moortools.close();'>x</a></div></div>"+
