...
sevenDeclare("moorCall.SoftphoneBar", null, {
  ...
  m7ChangedPeerToolBarState: function(peerState) {
    ...
    if(this.m7SrcNodePeerStateDom != null) {
       if (prePhonebarStatus && isCtiRelogin) {
           ...
           var rlZt = document.getElementById('rlZt');
           if(rlZt){rlZt.innerHTML = "";window.sessionStorage.setItem('rlZt','');}//容联状态
           ...
           if(!(currentEventObj.Event == "PeerStatus" && currentEventObj.Exten == this.m7Phone.sipNo && currentEventObj.PeerStatus == "Registered")) {
               ...
               var rlZt = document.getElementById('rlZt');
               if(rlZt){rlZt.innerHTML = "";window.sessionStorage.setItem('rlZt','');}
  ...
  m7ChangedCallToolBarState: function(state) {
    ...
    if(this.m7SrcNodePeerStateDom != null && state != "stInvalid") {
      console.log('当状态发生改变，并且状态不为空闲的时候会触发，' ,this.m7CallStateDescription[state] ,state)
      console.log("<span style=color:"+this.m7CallStateColor["call"]+">" + this.m7CallStateDescription[state] + "</span>")
      window.eventBus.$emit('m7CallStateDescription_status', state);  //工作台状态
      ...
      var rlZt = document.getElementById('rlZt');
      //失效状态下显示（-1）
      if(rlZt && this.m7CallStateDescription[state] == '失效'){
        rlZt.innerHTML = "<span style=color:#a7b2c0;>" + " (-1)" + "</span>";
        window.sessionStorage.setItem('rlZt','-1');
      }
  ...
  m7exConsult: function(phoneNum) {
    ...
    m7AutoBusyTime: function(AutoBusyTime) {
        if(this.m7SrcNodePeerStateDom != null) {
            ...
            var rlZt = document.getElementById('rlZt');
            rlZt.innerHTML = "";
            window.sessionStorage.setItem('rlZt','');
        }
        var self = this;
        if(m7PeerCalculagraph != null) {
            window.clearInterval(m7PeerCalculagraph);
  ...
  dialout: function (phoneNum, type, dccGroupId) {
    if (/^\d+$/.test(phoneNum)) {
      this.m7Phone.dialout(phoneNum, type, dccGroupId);
      return true;
    } else {
      moorCall.moortools.error("请输入正确的电话号码");
      return false;
...
sevenDeclare("moorCall.callProcessor", null, {
  ...
  EvtConnected: function(data) {
    ...
    console.log('接听方法: EvtConnected', phoneJson)
    window.eventBus.$emit('objectDeclareFunc', 'EvtConnected');
    window.localStorage.setItem('objectDeclareFunc', 'EvtConnected');
    window.eventBus.$emit("objectDeclareFuncRLBus", {type: 'EvtConnected', phoneJson: phoneJson}); //通知到phoneRL组件
  ...
  onHangup: function(data) {
    ...
    console.log('挂机方法：onHangup', phoneJson)
    window.eventBus.$emit('objectDeclareFunc', 'onHangup');
    window.eventBus.$emit('m7CallStateDescription_status', 'onHangup');
    window.localStorage.setItem('objectDeclareFunc', 'onHangup');
    window.localStorage.setItem('stateActionRL', 'onHangup')
    window.eventBus.$emit("objectDeclareFuncRLBus", {type: 'onHangup', phoneJson: phoneJson});
    this.sendAction(phoneJson);
  },
  ...
  onDialing: function(data) {
    ...
    console.log('外呼振铃:onDialing', phoneJson)
    window.eventBus.$emit('objectDeclareFunc', 'onDialing');
    window.localStorage.setItem('stateActionRL', 'onDialing');
    window.eventBus.$emit("objectDeclareFuncRLBus", {type: 'onDialing', phoneJson: phoneJson});
    this.sendAction(phoneJson);
  },
  m7Ring: function (data) {
    ...
    console.log('来电弹屏：m7Ring', phoneJson)
    // window.eventBus.$emit('objectDeclareFunc', 'm7Ring');
    window.eventBus.$emit('objectDeclareFunc2', phoneJson);
    window.localStorage.setItem('objectDeclareFunc', 'm7Ring');
    window.eventBus.$emit("objectDeclareFuncRLBus", {type: 'm7Ring', phoneJson: phoneJson});
    this.sendAction(phoneJson);
  },
