<template>
    <div class="page">
        <div class="clues-deal-page" :key="key">
          <div class="clues-deal-top">
            ...
            <div class="clues-deal-title">
              ...
              <div class="clues-deal-tools">
                  <div class="dialog-title-left">
                      <span class="dialog-title-bt dialog-title-text">{{cusTitle}}<span v-if="$route.query.total && $route.query.total > 0">({{$route.query.total}})</span></span>
                      ...
                      <div class="dialog-title-right">
                          <el-button size="mini" round plain class="plain-btn-style" @click="transferClue()" :disabled="isSaveFlag">
                ...
                <el-row :gutter="22" style="height: 100%">
                    ...
                    <el-col :span="6" class="padding0 flex-box-col flex-box-col-2">
                        <div class="dialog-center-temp customer-files">
                            <!-- <div class="lt-title">客户档案
                              <div v-if="isView">
                                <span class="customer-files-btn edit-btn">编辑</span>
                              </div>
                              <div v-else>
                                <span class="customer-files-btn save-btn">保存</span>
                                <span class="customer-files-btn cancle-btn">取消</span>
                              </div>
                            </div> -->
                            <div class="lt-content">
                              <!-- <CustomerFiles :key="time" v-if="archiveInfo" ref="archiveRef" :data="archiveInfo" :type="followType" @addIntentionInfoFn="addIntentionInfoFn" :followFormData="followFormData" @requeryDataFn="requeryDataFn" :areaList="areaList"></CustomerFiles> -->
                                  ...
                                  <el-row :gutter="22" class="customer-form">
                                    ...
                                    <el-col :span="24" class="m-b-10 store-and-car" :class="{'store-and-car-oth': index!=0}" v-for="(items, index) in intentionArchive" :key="index+'archive'">
                                      ...
                                      <el-row class="rowmbottom10">
                                        ...
                                        <el-col class="width-right-cont">
                                          <div class="grid-content grid-content-right" v-if="!editable">
                                            <span>{{archiveInfo.intentionInfo[index].areaName}}</span>
                                          </div>
                                          <div class="grid-content" v-else>
                                            <el-select
                                      ...
                                      <el-row class="rowmbottom10" style="margin-top: 10px;">
                                        ...
                                        <el-col class="width-right-cont">
                                          <div class="grid-content grid-content-right" v-if="!editable">
                                            <span>{{archiveInfo.intentionInfo[index].modelName}}</span>
                                          </div>
                                          <div class="grid-content" v-else>
                                            <el-select
                                                style="width:100%;"
                                      ...
                                      <el-row class="rowmbottom10" style="margin-top: 10px;">
                                        ...
                                        <el-col class="width-right-cont">
                                          <div class="grid-content grid-content-right" v-if="!editable">
                                            <span>{{archiveInfo.intentionInfo[index].fromTopText}} - {{archiveInfo.intentionInfo[index].fromSecondText}}</span>
                                          </div>
                                          <div class="grid-content" v-else>
                                            <el-select
                                                :disabled="index < archiveInfo.intentionInfo.length"
                                            ...
                                            >
                                                <el-option
                                                    v-for="item in intentionArchiveOptions[index].source_second"
                                                    :key="item.id"
                                                    :label="item.title"
                                                    :value="item.id"
                                                ></el-option>
                    ...
                    <el-col :span="6" class="padding0 flex-box-col flex-box-col-3">
                      <div class="dialog-center-temp add-follow">
                            ...
                            <div class="lt-content">
                                <AddFollow :key="addFollowKey" :type="followType" ref="followRef" :followFormData="followFormData" :followArchiveData="followArchiveData" :intentionInfo="catchIntentionInfo"></AddFollow>
                            </div>
                        </div>
                        <div class="dialog-center-temp follow-record">
                            <!-- <div class="lt-title">跟进记录</div> -->
                            <div class="lt-content scroll-area">
                                <FollowRecord :key="time" :type="followType" :areaArchive="areaArchive" :followFormData="followFormData"></FollowRecord>
                            </div>
                        </div>
                    </el-col>
            ...
            <div class="clues-deal-footer">
                ...
                <el-button size="medium" type="primary" class="w-90" @click="save" round :disabled="isSaveFlag">保存</el-button>
        ...
        <!-- 线索转移弹窗 -->
        <TransferClueDialog v-if="dialogVisibleTransferClue" @transferClueCloseFn="transferClueCloseFn" :type="followType" :clueInfo="clueInfo"></TransferClueDialog>
...
export default {
  components: {CallOutTemp,CallOutTempTR,CallOutTempRL,TransferClueDialog,OpenTask,ClueStore,CustomerFiles,AddFollow,FollowRecord,ClueEm,KnowledgeBase},
  data() {
    return {
      dialogVisible: true,
      val: "",
      ...
      areaList: [], //门店下拉数据
      time: '',
      followArchiveData: '', //潜客跟进详情

      followPage: {}, //下一条/上一条的id值
      areaArchive: [],  //跟进记录模块-门店档案
      key: '',
      isSaveFlag: false,  //为true代表保存成功过，保存按钮、线索转移、线索无效、稍后跟进按钮置灰，上一条下一条不再提示弹窗
  ...
  created() {
    ...
    if(this.$route.query.type == '0'){
      this.cusTitle = '逾期潜客跟进';
    }else if(this.$route.query.type == '1'){
      this.cusTitle = '今日潜客跟进';
    }else if(this.$route.query.type == '2'){
      this.cusTitle = '明日潜客跟进';
    }else if(this.$route.query.type == '3'){
      this.cusTitle = '后日潜客跟进';
    }
    this.getFollowForm();
    this.getAreaList();
    if(this.$route.query.id){
      this.getFollowArchive('init');
    }else{
      this.archiveInfo = {
  ...
  methods: {
    ...
    saveGj() {
      ...
      params.dccArchiveId = this.$route.query.id;
      params.userId = this.$route.query.userId;
      params.level = followFilters.level;
      if(followFilters.level == 'O'){
        params.remark = followFilters.remark;
      }else if(followFilters.level == 'F'){
        params.defeatReasonId = followFilters.defeatReasonId;
        params.remark = followFilters.remark;
      }else{
        Object.assign(params, followFilters);
        if(params.followResult == 1){
          params.nextFollowDate = this.$refs.followRef.nextFollowDate;
          if(followFilters.level != 'O' && followFilters.level != 'F'){
      ...
      if(followFilters.level == 'F' && !params.defeatReasonId){
        this.$message.error("请填写战败原因");
        return;
      }
      ...
      this.followIntentionAdd(params);
      
      this.saveOpenTask();
    },
    //保存跟进
    followIntentionAdd(params){
      Http.followIntentionAdd(params)
      .then(res => {
        this.$handleResponse(res, res => {
          ...
          if(!this.$route.query.back){
            //回潜客详情
            this.$router.push({'path':'/business/potential/detail', query: {'id': this.$route.query.id}});
          }else{
            this.getFollowArchive();
          }
    ...
    //线索跟进 - 线索详情接口
    getFollowArchive(init=false){
      if(!init) {
        console.log(this.$route.query.total)
        if (this.$route.query.total <= 1) {
          this.$router.replace({
            path: '/work',
          })
        }
      ...
      let params = {
        ...
        userId: this.$route.query.userId,
        pageFrom: this.$route.query.total ? '' : -1
      }
      ...
      Http.getFollowArchive(params)
        .then(res => {
          this.$store.commit("closeLoading");
          this.$handleResponse(res, res => {
            // 数据处理
            this.followArchiveData = res;
            this.openTask = res.openTask;
            this.clueInfo = res.archiveInfo.intentionInfo;
            this.intentionInfo = [];
            this.extendData = [];
            this.catchIntentionInfo = [];
            this.catchExtendData = [];
            this.clueInfo.forEach(element => {
            ...
            if(this.catchExtendData && this.catchExtendData[0]){
              this.competeModels = this.catchExtendData[0]['competeModel']
            }else{
              this.competeModels = res.archiveInfo.intentionInfo;
            }
            
            this.archiveInfo = res.archiveInfo;
            this.followPage = res.followPage;
            this.areaArchive = res.areaArchive;
            ...
            this.archiveInfo.intentionInfo.forEach((element,index) => {
              ...
              this.customersView = Object.assign({},this.archiveInfo);
              this.customersView.gender = this.archiveInfo.gender == 1? '男' : '女';
    ...
    handleData(data){
      if(data.intentionId && data.userId){
        this.$router.replace({
          path: '/_empty',
          query:{'back': this.$route.query.back,'total': this.$route.query.total,'type': this.$route.query.type}
        })
        this.$router.replace({ path: '/work/customerFollow',query:{'back': this.$route.query.back,'total': this.isSaveFlag ? this.$route.query.total - 1 : this.$route.query.total, 'type': this.$route.query.type, 'id': data.intentionId, 'userId': data.userId}})
        this.getFollowArchive();
    ...
    sourceChange(index, noclear) {
      ...
      for (let i of this.clueFromOptions) {
        if (i.id == this.intentionArchive[index].fromTopId) {
          this.intentionArchiveOptions[index].source_second = i.children || [];
          //只有一级来源的时候也可以提交
          if(this.intentionArchiveOptions[index].source_second.length == 0 && this.intentionArchive[index].modelId){
            //提交新增的门店车型
    ...
    initData(){
      this.catchIntentionInfo = [];
      this.catchExtendData = [];
      this.archiveInfo.intentionInfo.forEach(element => {
        this.intentionInfo.push(element)
        this.extendData.push(element.extend)
        this.catchIntentionInfo.push(element);  //会更新的数据
        this.catchExtendData.push(element.extend);
      ...
      if(this.catchExtendData && this.catchExtendData[0]){
        ...
        this.competeModels = this.archiveInfo.intentionInfo;
      }
      this.customers = {
        nickname: this.archiveInfo.nickName || this.archiveInfo.nickname,
        gender: this.archiveInfo.gender, //性别，1男，2女
